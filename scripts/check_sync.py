...
def planner_line_version(package: Path) -> str | None:
    text = (package / "SKILL.md").read_text(encoding="utf-8")
    match = re.search(r"Planner: robust-feature-planner v(\S+?) ", text)
    return match.group(1) if match else None
...
def check_versions() -> list[str]:
    ...
    problems = []
    for name, package, version in (("claude", CLAUDE, claude_version), ("codex", CODEX, codex_version)):
        body_version = planner_line_version(package)
        if body_version is None:
            problems.append(f"{name} SKILL.md is missing the 'Planner: robust-feature-planner v…' Document Control line.")
        elif body_version != version:
            problems.append(
                f"{name} SKILL.md Planner line says v{body_version} but frontmatter says v{version} - update both together."
            )
    return problems
