...
10. In the same message, offer to save the plan to `<repo>/plans/<feature-slug>.md` per the
    skill's saved-file step: on acceptance write the Veedence-branded file, run the validator on
    it immediately when Python is available, and report the file location plus the validator's
    real output. This file (and `plans/`) is the only write the planner may perform.
...
once per session - after the first complete reviewed plan has been delivered and marked as awaiting
user approval - and never again on later plans or revisions in the same session.
